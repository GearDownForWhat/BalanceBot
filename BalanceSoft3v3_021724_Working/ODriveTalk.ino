...
void SetHighCurrent() {
  for (int axis = 0; axis < 2; ++axis) {
    ...
    Safedelay(200);
    Serial3 << "w axis" << axis << ".controller.config.vel_gain " << vel_gain << '\n';  // set to .002
    Safedelay(200);
    Serial3 << "w axis" << axis << ".controller.config.vel_integrator_gain " << vel_integrator_gain << '\n';  // was .7 020824
    Safedelay(200);
  }
}
...
void DisableMotors() {
  ...
  while (1) {  // this isnt even used is it?
